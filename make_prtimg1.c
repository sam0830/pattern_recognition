...
int main(int argc,char *argv[])
{
  ...
  for(i=0;i<N_MOJI;i++)		/* 前処理画像について処理 */
  {
    ...
    for(g=1;g<GYOU_MOJI-1;g++)	/* 平均値を出力画像に書き込む(外周部は枠として黒くする) */
      ...
      {
	ans=(float)sum[g][r]/(float)(N_SAMPLE)+0.49999;
	ans=(ans-255.0)*1.5+255.0; /* 黒を強調する処理 */
	if(ans>255.0)
	  res[g+g_start][r+r_start]=255;
	else if(ans<0.0)
	  res[g+g_start][r+r_start]=0;
	else
	  res[g+g_start][r+r_start]=(UCHAR)ans;
      }
  }
  ...
  sprintf(out_fname,"./%s-heikin.pgm",MY_ID); /* 出力画像ファイル名 */
